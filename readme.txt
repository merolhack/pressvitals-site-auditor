...
* **Headless / API-first.** Core Site Health has no public report API. PressVitals
...
* **Scheduled + alerting.** Core never runs on a schedule and never tells you
  when something breaks. PressVitals runs daily via WP-Cron and **emails the admin
...
* **Pluggable + configurable.** Core's tests are fixed. PressVitals lets any plugin
...
* `pvsa_registered_checks` — register/override probes.
* `pvsa_setting_{key}` — override a stored threshold at read time.
* `pvsa_alert_email` — change the failure-alert recipient.
* `pvsa_http_timeout`, `pvsa_disk_free_min_bytes`, `pvsa_memory_min_bytes`,
  `pvsa_fatal_lookback_hours`, `pvsa_fatal_scan_max_bytes` — tune environment probes.
* `pvsa_ssl_warn_days`, `pvsa_ssl_fail_days` — TLS expiry thresholds.
* `pvsa_backup_warn_days`, `pvsa_backup_fail_days` — backup-recency thresholds.
* `pvsa_last_backup_timestamp` — report your last successful backup time (UNIX) so
  the backup probe works with *any* backup plugin, host, or off-site service.
* `pvsa_backup_plugins` — list of backup-plugin basenames recognised by presence.
* `pvsa_max_expired_transients`, `pvsa_max_revisions`, `pvsa_max_spam_comments` —
  database-bloat thresholds.
* `pvsa_known_tables` — full table names to treat as expected (silences the
  non-core/orphaned-tables probe for legitimate plugin tables).
* `pvsa_orphan_tables_warn` — non-core table count above which the probe warns.
* `pvsa_sending_domain` — domain used for the SPF/DMARC lookup.

= Compatibility =

PressVitals has **no plugin dependencies** and runs on virtually any WordPress
...
1. Upload the `pressvitals-site-auditor` folder to `/wp-content/plugins/`, or install
   it from the Plugins screen.
2. Activate the plugin through the **Plugins** menu in WordPress.
3. Go to **Tools → PressVitals Site Auditor** to view the report and configure
   thresholds.
...
* Implement the final P3 probes: https_mixed_content and rest_api_reachable.
  ...
  tables (filterable via `pvsa_known_tables` / `pvsa_orphan_tables_warn`). Total
  built-in probes: 29.
...
= 1.0.0 =
* Initial release: pluggable probe engine (`pvsa_registered_checks` filter), 22
